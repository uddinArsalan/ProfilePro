    ...
    <div className="bg-[#363C43] rounded-3xl p-4 sm:p-6 shadow-2xl w-full max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row items-center justify-between mb-4 sm:mb-8 space-y-4 sm:space-y-0">
        <div className="flex space-x-3 sm:space-x-5 items-center">
          ...
          <h2 className="text-sm sm:text-base font-semibold text-white bg-[#171717] px-4 sm:px-8 py-2 sm:py-3 rounded-full shadow-lg">
            Gallery
          </h2>
        </div>
        <label className="flex items-center bg-[#3F454C] text-gray-300 px-3 sm:px-5 py-2 sm:py-3 rounded-full cursor-pointer hover:bg-gray-700 transition-all duration-300 shadow-lg hover:shadow-xl active:bg-teal-700">
      ...
      >
        {images.map((image, index) => (
          <div
            key={index}
            className="flex-shrink-0 w-32 h-32 sm:w-44 sm:h-44 bg-gray-800 rounded-xl sm:rounded-2xl overflow-hidden transform transition-all duration-300 hover:scale-105 sm:hover:scale-110 hover:rotate-2 hover:shadow-2xl"
          >
            <img
              src={image.src}
              alt={image.alt}
